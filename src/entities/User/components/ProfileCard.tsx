"use client";
import Image from "next/image";
import { useEffect, useState } from "react";

import { HeadPhoneLogo, PlayLogo, VerifiedLogo, useAudio } from "@/shared";

import { Button } from "@/shared";
import { IPodcast } from "@/entities";

const ProfileCard = ({
  podcastData,
  imageUrl,
  nikname,
}: {
  podcastData: IPodcast[],
  imageUrl: string,
  nikname: string
}) => {
  const { setAudio } = useAudio();

  const [randomPodcast, setRandomPodcast] = useState<IPodcast | null>(null);

  const playRandomPodcast = () => {
    const randomIndex = Math.floor(Math.random() * podcastData.length);

    setRandomPodcast(podcastData[randomIndex]);
  };

  useEffect(() => {
    if (randomPodcast) {
      setAudio({
        title: randomPodcast.podcastTitle,
        audioUrl: randomPodcast.audioUrl || "",
        imageUrl: randomPodcast.imageUrl || "",
        author: randomPodcast.author,
        podcastId: randomPodcast._id,
      });
    }
  }, [randomPodcast, setAudio]);

  return (
    <div className="mt-6 flex flex-col gap-6 max-md:items-center md:flex-row">
      <Image
        src={imageUrl}
        width={250}
        height={250}
        alt="Podcaster"
        className="aspect-square rounded-lg"
      />
      <div className="flex flex-col justify-center max-md:items-center">
        <div className="flex flex-col gap-2.5">
          <figure className="flex gap-2 max-md:justify-center">
            <Image
              src={VerifiedLogo}
              width={15}
              height={15}
              alt="verified"
            />
            <h2 className="text-14 font-medium text-white-2">
             Создатель подкастов
            </h2>
          </figure>
          <h1 className="text-32 font-extrabold tracking-[-0.32px] text-white-1">
            {nikname}
          </h1>
        </div>
        <figure className="flex gap-3 py-6">
          <Image
            src={HeadPhoneLogo}
            width={24}
            height={24}
            alt="headphones"
          />
        </figure>
        {podcastData?.length > 0 && (
          <Button
            onClick={playRandomPodcast}
            className="text-16 bg-orange-1 font-extrabold text-white-1"
          >
            <Image
              src={PlayLogo}
              width={20}
              height={20}
              alt="random play"
            />{" "}
            &nbsp; Включить случайный подкаст
          </Button>
        )}
      </div>
    </div>
  );
};

export default ProfileCard;